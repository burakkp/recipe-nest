...
recipe straight from a shared link — a Cloudflare Worker resolves the link and an LLM
structures it into ingredients and steps.
...
| LLM               | Workers AI — Llama 3.3 70B (free)  | Swap the body of `callLLM` in `worker/src/index.js` (commented Claude stub included) |
...
- **Workers AI free tier caps at ~10,000 neuron-inferences/day.** Fine for personal use and
  demos; will need a paid plan or a different model under heavier use.
